...
use core::{fmt::Display, marker::PhantomData, ptr};
...
/// Create a `seq_file` in [`debugfs`] under the `parent` directory.
pub fn debugfs_create_with_parent<T: SeqOperations>(
    name: &CStr,
    ...
    <T as SeqOperations>::OpenData: 'static,
{
    ...
    unsafe {
        DebugFsFile::create_with_parent(name, parent, data, &SeqFileOperationsVTable::<T>::VTABLE)
    }
}

/// Create a `seq_file` at the top level of [`debugfs`].
pub fn debugfs_create_toplevel<T: SeqOperations>(
    name: &CStr,
    data: T::OpenData,
    ...
    <T as SeqOperations>::OpenData: 'static,
{
    ...
    // is safe to call only if the vtable is valid when used on an inode with
    // `data::into_pointer` stored in `i_private`.
    Ok(unsafe { DebugFsFile::create_toplevel(name, data, &SeqFileOperationsVTable::<T>::VTABLE) }?)
}
