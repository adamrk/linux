...
macro_rules! make_param_ops {
    ...
    ($(#[$meta:meta])* $ops:ident, $ty:ty) => {
        ...
        pub static $ops: kernel::bindings::kernel_param_ops = kernel::bindings::kernel_param_ops {
            flags: if <$ty as kernel::module_param::ModuleParam>::NOARG_ALLOWED {
                kernel::bindings::KERNEL_PARAM_OPS_FL_NOARG
            } else {
                0
            },
            set: Some(<$ty as kernel::module_param::ModuleParam>::set_param),
            get: Some(<$ty as kernel::module_param::ModuleParam>::get_param),
            free: Some(<$ty as kernel::module_param::ModuleParam>::free),
        };
    };
}

macro_rules! internal_make_param_ops {
    ($(#[$meta:meta])* $ops:ident, $ty:ty) => {
        $(#[$meta])*
        ///
        /// Static [`kernel_param_ops`](../../../include/linux/moduleparam.h)
...
impl<T, const N: usize> ArrayParam<T, { N }> {
    pub fn values(&self) -> &[T] {
        unsafe {
            core::mem::transmute::<&[core::mem::MaybeUninit<T>], &[T]>(&self.values[0..self.used])
        }
    }
}
