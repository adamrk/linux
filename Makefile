...
KBUILD_RUSTCFLAGS += -C debug-assertions=y
else
KBUILD_RUSTCFLAGS += -C debug-assertions=n
endif

ifdef CONFIG_RUST_OVERFLOW_CHECKS
KBUILD_RUSTCFLAGS += -C overflow-checks=y
else
KBUILD_RUSTCFLAGS += -C overflow-checks=n
endif
...
KBUILD_RUSTCFLAGS += -C opt-level=$(KBUILD_RUSTCFLAGS_OPT_LEVEL_MAP)
else ifdef CONFIG_RUST_OPT_LEVEL_0
KBUILD_RUSTCFLAGS += -C opt-level=0
else ifdef CONFIG_RUST_OPT_LEVEL_1
KBUILD_RUSTCFLAGS += -C opt-level=1
else ifdef CONFIG_RUST_OPT_LEVEL_2
KBUILD_RUSTCFLAGS += -C opt-level=2
else ifdef CONFIG_RUST_OPT_LEVEL_3
KBUILD_RUSTCFLAGS += -C opt-level=3
else ifdef CONFIG_RUST_OPT_LEVEL_S
KBUILD_RUSTCFLAGS += -C opt-level=s
...
KBUILD_RUSTCFLAGS += -C force-frame-pointers=y
else
...
DEBUG_RUSTCFLAGS += -C debuginfo=1
else
DEBUG_RUSTCFLAGS += -C debuginfo=2
endif
endif
